...
. Follow the instructions to setup the {Tailscale-Online-Target} for the node where pictures will be uploaded.
After installing the necessary files, the following command enables the Tailscale Online Target for the `meerkat` node where my photos will be uploaded.
The Tailscale Online Target for this specific instance is added as a dependency for the systemd service that uploads the photos.
+
[,sh]
----
...
Rclone::
+
.. Create the Rclone configuration directory `~/.config/rclone`.
+
[,sh]
----
sudo mkdir --parents ~/.config/rclone/
----

.. Configure the Rclone credentials in `~/.config/rclone/rclone.conf`.
+
...
+
.~/.config/systemd/user/upload-pictures.service
[,systemd]
----
include::systemd/user/upload-pictures.service[]
----
...
[,sh]
----
systemctl --user enable --now upload-pictures.path
----

. Symlink the systemd units to the `~/.config/systemd/user/` directory.
+
[,sh]
----
ln --relative --symbolic systemd/user/* ~/.config/systemd/user/
----

. Enable and start the `pi-camera.service`` and `upload-pictures.path` systemd units.
The latter unit will automatically run the similarly named service whenever new pictures are added to the `~/Pictures` directory.
+
[,sh]
----
...
* I should probably document disabling password-based SSH authentication.
